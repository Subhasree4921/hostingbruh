...
from .forms import AnswerForm, CustomUserCreationForm, QuestionForm, ProfileForm
...
def home(request):
    if "q" in request.GET:
        q = request.GET["q"]
        quests = (
            Question.objects.annotate(total_comments=Count("answer__comment"))
...
def detail(request, id):
    ...
    return render(
        request,
        "detail.html",
        {"quest": quest, "tags": tags, "answers": answers, "answerform": answerform},
    )
...
def save_comment(request):
    if request.method == "POST":
        ...
        user = request.user
        Comment.objects.create(answer=answer, comment=comment, user=user)
